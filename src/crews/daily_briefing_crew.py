...
class DailyBrieferCrew:
    ...
    def __init__(self, target_date: date, verbose: bool = True):
        """
    ...
    def _setup_crew(self):
        ...
        data_collection_task = create_data_collection_task(self.target_date, verbose=self.verbose)
        email_analysis_task = create_email_analysis_task(self.target_date, verbose=self.verbose)
        email_briefing_task = create_email_briefing_task(self.target_date, verbose=self.verbose)
        calendar_analysis_task = create_calendar_analysis_task(self.target_date, verbose=self.verbose)
        task_analysis_task = create_task_analysis_task(self.target_date, verbose=self.verbose)
        document_assembly_task = create_document_assembly_task(self.target_date, verbose=self.verbose)
...
def create_daily_briefer_crew(target_date: date, verbose: bool = True) -> DailyBrieferCrew:
    """
    ...
    """
    return DailyBrieferCrew(target_date, verbose=verbose)